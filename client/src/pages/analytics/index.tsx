...
const PALETTE = ["#818cf8", "#34d399", "#fbbf24", "#f472b6", "#60a5fa", "#f87171"];
    ...
    <div className="bg-slate-900/95 backdrop-blur border border-slate-700/60 rounded-2xl px-4 py-3 shadow-2xl">
      ...
      {payload.map((p: any, i: number) => (
        <p
          ...
          style={{ color: p.color ?? p.fill ?? "#818cf8" }}
        >
...
export default function Analytics() {
  ...
  orders.forEach((o: any) => {
    (o.items ?? []).forEach((item: any) => {
      const cat =
        products.find((p: any) => p.title === item.title)?.category ?? "Other";
      catMap.set(
        cat,
        (catMap.get(cat) ?? 0) + Number(item.price ?? 0) * Number(item.qty ?? 0)
      );
    });
  });
  ...
  const cancelledCnt = orders.filter((o: any) => o.status === "Cancelled").length;
  const returnedCnt = orders.filter((o: any) => o.status === "Returned").length;
  const deliveryOrders = orders.filter(
    (o: any) => o.delivery_type === "delivery"
  ).length;

  const deliveryRate = orders.length
    ? Math.round((deliveredCnt / orders.length) * 100)
    : 0;
  ...
  return (
    <AdminLayout>
      ...
      <div className="space-y-5 pb-12 px-0.5">
          ...
          <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full bg-white/80 border border-indigo-200/60 px-3 py-1 text-xs font-bold text-indigo-600 mb-3 shadow-sm">
                <Sparkles className="w-3 h-3" /> Offline Store Analytics
              </div>
              <h1 className="text-2xl md:text-[1.85rem] font-black tracking-tight text-slate-800">
                Store Analytics
              </h1>
              <p className="text-sm text-slate-500 mt-1.5">
                Track revenue, order flow, delivery performance, and top products.
              </p>
            </div>
        ...
        <div
          className="page-in grid grid-cols-2 lg:grid-cols-4 gap-3"
        ...
        >
          {[
            ...
            {
              label: "Avg. Order",
              value: `${avgOrder.toLocaleString()} dh`,
              icon: ShoppingCart,
            ...
            >
              <div
                className={`w-10 h-10 rounded-xl bg-gradient-to-br ${grad} flex items-center justify-center shadow-sm shrink-0`}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <XAxis
                    ...
                    axisLine={false}
                    tickLine={false}
                    tick={{ fontSize: 11, fill: "#a5b4fc", fontWeight: 700 }}
                    dy={8}
                  />
                  <YAxis
                    axisLine={false}
          ...
          <div
            ...
            style={{
              ...
              background:
                "linear-gradient(145deg,#064e3b 0%,#065f46 50%,#064e3b 100%)",
            }}
          >
            <div className="px-5 pt-5 pb-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-emerald-300/80 mt-0.5">
                  Top {revByCategory.length} categories
                </p>
              </div>
              <div className="w-9 h-9 rounded-xl bg-white/10 flex items-center justify-center">
              ...
              <div className="flex-1 space-y-2.5 overflow-hidden">
                {revByCategory.length === 0 ? (
                  ...
                  revByCategory.map((item, i) => {
                    const pct = totalCatRev
                      ? Math.round((item.value / totalCatRev) * 100)
                      : 0;
                    return (
                      <div key={i} className="legend-row cursor-default">
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span className="text-xs font-semibold text-white/80 truncate max-w-[100px]">
                              {item.name}
                            </span>
                          </div>
            ...
            <div className="px-5 pt-5 pb-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-amber-300/80 mt-0.5">
                  Distribution across all offline statuses
                </p>
              </div>
              <div className="w-9 h-9 rounded-xl bg-white/10 flex items-center justify-center">
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <XAxis
                    ...
                    axisLine={false}
                    tickLine={false}
                    tick={{ fontSize: 10, fill: "#d6d3d1", fontWeight: 700 }}
                    dy={8}
                  />
                  <YAxis
                    axisLine={false}
                  ...
                  <Tooltip
                    ...
                    cursor={{ fill: "rgba(255,255,255,.04)", radius: 6 }}
                  />
          ...
          <div
            ...
            style={{
              ...
              background:
                "linear-gradient(145deg,#1e0a3c 0%,#2d1065 50%,#1e0a3c 100%)",
            }}
          >
            <div className="px-5 pt-5 pb-3 flex items-center justify-between">
              <div>
                      ...
                      <div key={i} className="legend-row cursor-default">
                        <div className="flex items-center justify-between mb-1.5">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold text-white/85 truncate max-w-[160px]">
                              {item.name}
                            </span>
                          </div>
