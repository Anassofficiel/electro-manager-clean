...
const STATUS_META: Record<string, { color: string; icon: any; bg: string }> = {
  ...
  New: { color: "text-amber-600", icon: Clock, bg: "bg-amber-100" },
  ...
  Returned: { color: "text-slate-600", icon: Package, bg: "bg-slate-200" },
};
...
const STATUS_HERO: Record<string, { grad: string; ring: string; text: string }> =
{
  ...
  Returned: {
    grad: "from-slate-50 via-white to-slate-100",
    ring: "ring-slate-200",
    text: "text-slate-700",
  },
};
...
function joinAddress(address1?: string | null, address2?: string | null) {
...
export default function OrdersList() {
  ...
  useEffect(() => {
    ...
    if (!printId || !safeOrders.length) return;

    const found = safeOrders.find((o: any) => String(o.id) === String(printId));
    if (found) {
  ...
  const handlePrint = () => {
    if (!selectedOrder?.id) return;
    markPrinted.mutate(selectedOrder.id);
    window.print();
  };

  const hero = STATUS_HERO[selectedOrder?.status] ?? STATUS_HERO["Confirmed"];

  const storeName = settings?.storeName || "ELECTRO MANAGER";
  const storePhone = settings?.phone || "";
  const storeEmail = settings?.email || "";
  const storeWebsite = settings?.website || "";
  const storeInstagram = settings?.instagram || "";
  const storeFacebook = settings?.facebook || "";
  const storeAddress = joinAddress(settings?.address1, settings?.address2);
  const invoiceFooter =
  ...
  return (
    <AdminLayout>
      ...
      <div className="space-y-5 pb-12 px-0.5 no-print">
          ...
          <div className="relative flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full bg-white/80 border border-indigo-200/60 px-3 py-1 text-xs font-bold text-indigo-600 mb-3 shadow-sm">
                <ShoppingBag className="w-3 h-3" /> Offline Orders Management
              </div>
              <h1 className="text-2xl md:text-[1.85rem] font-black tracking-tight text-slate-800">
                All Orders
              </h1>
              <p className="text-sm text-slate-500 mt-1">
                Manage, search, print, and update your store orders in real time.
              </p>
            </div>
        ...
        <div
          className="page-in flex gap-2 overflow-x-auto pb-1"
        ...
        >
          {STATUS_TABS.map((tab) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-3.5 h-3.5 ${isActive ? meta.color : "text-slate-400"
                    }`}
                />
                {tab}
                <span
                  className={`text-xs font-black px-1.5 py-0.5 rounded-lg ${isActive
                      ? `${meta.bg} ${meta.color}`
        ...
        <div className="page-in" style={{ animationDelay: "100ms" }}>
          <div className="search-wrap flex gap-3 items-center bg-white/90 backdrop-blur border border-slate-200/80 rounded-2xl p-2 shadow-sm transition-all duration-300">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search by order code, customer, phone, city or delivery type…"
                value={search}
          ...
          {groupedOrders.length === 0 ? (
            ...
            groupedOrders.map((group, groupIndex) => (
              ...
              >
                ...
                <div className="hidden lg:grid grid-cols-[150px_1.4fr_180px_170px_190px_120px_44px] gap-4 px-5 py-3 bg-slate-50/70 border-b border-slate-100">
                  {["Order", "Customer", "Date", "Status", "Payment", "Total", ""].map(
                    (label, i) => (
                      <div
                        key={i}
                        className={`text-[11px] font-black uppercase tracking-widest text-slate-400 ${i === 5 ? "text-right" : "text-left"
                          }`}
                      >
                        {label}
                      </div>
                ...
                <div className="p-3 md:p-4 space-y-3">
                  {group.orders.map((order: any, idx: number) => (
                    ...
                    >
                          ...
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              <p className="font-black text-slate-800 truncate">
                                {order.order_code || `#${order.id}`} • {order.customerName}
                              </p>
                        ...
                        <div className="grid grid-cols-2 gap-3 pt-1">
                          <div>
                            ...
                            <p className="text-xs text-slate-400">
                              {format(getSafeDate(order.date), "HH:mm")}
                            </p>
                          </div>
                      ...
                      <div className="hidden lg:grid grid-cols-[150px_1.4fr_180px_170px_190px_120px_44px] gap-4 items-center">
                        <div>
          ...
          {filtered.length > 0 && (
            <div className="px-2">
              <p className="text-xs text-slate-400 font-medium">
                Showing <span className="font-black text-slate-600">{filtered.length}</span>{" "}
                of <span className="font-black text-slate-600">{safeOrders.length}</span>{" "}
                orders across{" "}
                <span className="font-black text-slate-600">{groupedOrders.length}</span>{" "}
                date group{groupedOrders.length > 1 ? "s" : ""}
              </p>
            </div>
          )}
        </div>
        ...
        <SheetContent className="w-full sm:max-w-md md:max-w-lg border-l border-slate-200/60 shadow-2xl p-0 flex flex-col bg-slate-50/95 backdrop-blur">
          {selectedOrder && (
            <div className="sheet-in flex flex-col h-full">
              <div
                className={`relative overflow-hidden bg-gradient-to-br ${hero.grad} border-b border-slate-200/60 no-print`}
              >
                    ...
                    <div className="hero-in">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <SheetTitle className="text-2xl font-black text-slate-800 tracking-tight">
                          {selectedOrder.order_code || `Order #${selectedOrder.id}`}
                        </SheetTitle>
                      </div>
                      <p className="text-xs text-slate-500 font-medium pl-10">
                        {format(
                          getSafeDate(selectedOrder.date),
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4 no-print">
                  ...
                  <div className="p-4 space-y-2.5">
                    {[
                      ...
                      <div key={i} className="flex items-start gap-3 group">
                        ...
                        <span
                          className={`text-sm leading-relaxed ${bold
                ...
                <div className="detail-card rounded-2xl bg-white border border-slate-100 overflow-hidden shadow-sm">
                  ...
                  <div className="p-4 space-y-2.5 text-sm">
                    {[
                      {
                        label: "Subtotal",
                        val: `${Number(selectedOrder.subtotal || 0).toLocaleString()} dh`,
                        cls: "text-slate-500",
                      },
                      {
                        label: "Shipping",
                        val: `${Number(selectedOrder.shipping || 0).toLocaleString()} dh`,
                        cls: "text-slate-500",
                      },
                    ...
                    <div className="pt-3 mt-1 border-t-2 border-dashed border-slate-100 flex justify-between items-center">
                      ...
                      <span className={`font-black text-xl ${hero.text}`}>
                        {Number(selectedOrder.total || 0).toLocaleString()} dh
                      </span>
                    </div>
              ...
              <div className="p-5 bg-white/95 border-t border-slate-200/60 no-print">
                ...
                <div className="grid grid-cols-1 gap-2">
                  <button
                    ...
                    className="status-btn w-full h-10 rounded-xl border border-slate-200 bg-slate-50 text-slate-700 hover:bg-slate-100 font-bold text-sm"
                  >
                    Mark as Returned
                  </button>
              ...
              <div className="print-area hidden print:block">
                <div className="border border-slate-200 rounded-2xl p-8">
                  <div className="flex items-start justify-between border-b pb-6 gap-8">
                    <div className="max-w-[60%]">
                      ...
                      {storeAddress && (
                        <p className="text-sm text-slate-500 mt-2">{storeAddress}</p>
                      )}

                      {storePhone && (
                        <p className="text-sm text-slate-500 mt-1">{storePhone}</p>
                      )}

                      {storeEmail && (
                        <p className="text-sm text-slate-500 mt-1">{storeEmail}</p>
                      )}

                      {storeWebsite && (
                        <p className="text-sm text-slate-500 mt-1">{storeWebsite}</p>
                      )}
                    </div>
                    ...
                    <div className="w-full max-w-sm space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">
                          {Number(selectedOrder.subtotal || 0).toLocaleString()} dh
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-500">Shipping</span>
                        <span className="font-medium">
                          {Number(selectedOrder.shipping || 0).toLocaleString()} dh
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-500">Discount</span>
                        <span className="font-medium">
                          - {Number(selectedOrder.discount || 0).toLocaleString()} dh
                        </span>
                      </div>
                  ...
                  {(storeWebsite || storeInstagram || storeFacebook) && (
                  ...
                  <div className="pt-6 border-t flex items-end justify-between">
                    <div>
                      ...
                      <div className="mt-10 w-48 border-b border-slate-400" />
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <div className="mt-10 w-48 border-b border-slate-400 ml-auto" />
                    </div>
                  </div>

                  <div className="mt-8 text-center text-xs text-slate-400 whitespace-pre-line">
                    {invoiceFooter}
                  </div>
                </div>
              </div>
